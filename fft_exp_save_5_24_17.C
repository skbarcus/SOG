...
//********* Histograms ********//
   ...
   TF1 *fsin = new TF1("fsin", "cos(2*pi*(3*(x)))*exp(-pi*pow((x),2.0))", -1.5, 1.5);

   //TF1 *fsin = new TF1("fsin", "cos(2*pi*(5*(x)))*exp(-pi*pow((x),2.0))", -1.5, 1.5);
   //TF1 *fsin = new TF1("fsin", "exp(-0.5*pow((x),2.0))", -1.5, 1.5);
   //TF1 *fsin = new TF1("fsin", "1./pow((1.+x),2.)", 1., 5.);
   //TF1 *fsin = new TF1("fsin", "cos(2*pi*2*x)", -1.5, 1.5);
   fsin->Draw();

   Int_t n=100;
   TH1 *hsin = new TH1D("hsin", "hsin", n+1, -1.5, 1.5);
   Double_t x;

   //Fill the histogram with function values
   for (Int_t i=0; i<=n; i++)
     {
       x = (-1.5+(Double_t(i)/n)*3.);
       hsin->SetBinContent(i+1, fsin->Eval(x));
       //cout<<"hsin["<<i<<"] = "<<hsin[i]<<endl;
       //cout<<"hsin["<<(i+1)<<"]   Value = "<<hsin->GetBinContent(i+1)<<endl;
       
	 //Int_t Entries = hsin->GetEntries();
	 //cout<<"Entries = "<<Entries<<endl;
	 //Double_t Value = hsin->GetBinContent(i+1);
       	 //cout<<"Value = "<<hsin->GetBinContent(i+1)<<endl;
     }
   ...
   TH1 *hm =0;
   //TH1 *hm = new TH1D("hm", "hm", n+1, -1.5, 1.5);
   ...
   for (Int_t i=0;i<n+1;i++)
     {
       ...
       Double_t Value = hm->GetBinContent(i+1)*(1./pow(n,0.5));
       //cout<<"hm["<<(i+1)<<"]   Value = "<<Value<<endl;
   ...
   //cout<<hm->GetXaxis()->GetNBinsX()<<endl;

   /*
   //TH1D hmrescaled("hmrescaled","Magnitude",hm->GetNBinsX(),hm->GetXaxis()->GetXmin()/hsin->GetXaxis()->GetXmin(),hm->GetXaxis()->GetXmax()/hsin->GetXaxis()->GetXmax());
   TH1D hmrescaled("hmrescaled","Magnitude",n+1,-1000./1.5,1000./1.5);
   for (Int_t i = 0; i<n; i++)
     {
       hmrescaled.SetBinContent(i+1,hm->GetBinContent(i));
       //cout<<hmrescaled->GetBinContent(i+1)<<endl;
     }
   */
...
//********* Data array - same transform ********//
  ...
  TH1 *FFT = new TH1D("FFT", "FFT", n+1, -(n+1)/(3.*2.), (n+1)/(3.*2.));
  ...
  for(Int_t i=0;i<n+2;i++)
    {
      if(i>inflection+1)
	{
	  FFT->SetBinContent(i-1-inflection,hm->GetBinContent(i)/(1./(3./(n+1.))));//*(1./pow(n,0.5)));
	}
    }
  ...
  for(Int_t i=0;i<n;i++)
   {
     if(i<=inflection)
       {
	 FFT->SetBinContent(inflection+i+1,hm->GetBinContent(i+1)/(1./(3./(n+1.))));//*(1./pow(n,0.5)));
       }
   }
