...
Int_t ngaus = 6;                        //Number of Gaussians used to fit data.
Double_t Z = 6.;                         //Atomic number C12.
Double_t A = 12.;                        //Mass number C12.
Double_t MtC = 12.0*0.9315;              //Mass of C12 in GeV.
Double_t gamma = 1.5/pow(3./2.,0.5);//1.2*pow(2.0/3.0,0.5);   //Gaussian width [fm] from Amroun gamma*sqrt(3/2) = 0.8 fm.
...
//Print the data read from the file. 
 ...
 //Make a new TGraph to plot on the canvas. Parameters are (number of points, x values array, y values array).
  graph = new TGraphErrors(nlines-skip,theta,sigexp,uncertainty);
...
*/
 ...
 //c2->SetLogy(); 
 graph->Draw();
 TF1 *fxs = new TF1("fxs",xs, ymin, ymax,ngaus);//ngaus*2);
...
*/
 ...
 if(userand == 0)
   {
     Double_t d = 0.5001;//0.5001
     
     fxs->SetParLimits(11,0.00000001,0.5);//0.5
