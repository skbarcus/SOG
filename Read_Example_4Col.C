...
void Read_Example_4Col() {
  ...
  FILE *fp = fopen("/home/skbarcus/Tritium/Analysis/SOG/Read_Example_4Col.txt","r");
  //Open file to write output to.
  std::ofstream output ("junk.txt", std::ofstream::out);
  
  Float_t x,y,z,m;          //Temporary variables to hold data from data file.
  ...
  Float_t xval[21];    //Arrays to hold X and Y data.
  Float_t yval[21];
  Float_t zval[21];
  Float_t mval[21];
  ...
  while (1) {
	...
	ncols = fscanf(fp,"%f %f %f %f",&x, &y, &z, &m);
	if (ncols < 0) break;    
	cout<<x<<"   "<<y<<"   "<<z<<"   "<<m<<endl;
	xval[nlines-skip] = x;
	yval[nlines-skip] = y;
  ...
  for(int i=0; i<40; i++)
    {
      cout<<"xval["<<i<<"] = "<<xval[i]<<"   yval["<<i<<"] = "<<yval[i]<<"   zval["<<i<<"] = "<<zval[i]<<"   mval["<<i<<"] = "<<mval[i]<<endl;
    }
