...
Double_t FB(float E0, float theta, Double_t *par)
{
  ...
  Double_t eta = 1 + (2*E0/Mt3H) * pow(sin(theta*deg2rad/2.0),2.);//Beck 1984.
  ...
  if(useFB_FM == 1)
    {
      //val = pow(Z,2.) * mottxs * (   (  pow(FB_FC_sum,2.)+tau*pow(FB_FM_sum,2.)  )/(1+tau) + 2*tau*pow(FB_FM_sum,2.)*pow(1/(1+rho),2.)*pow(1/tan(theta*deg2rad),2.)  );  //Try to fit FC and FM. This was for the recoil proton being measure.
      //val = mottxs * (   (  pow(FB_FC_sum,2.)+tau*pow(FB_FM_sum,2.)  )/(1+tau) + 2*tau*pow(FB_FM_sum,2.)*pow(tan(theta*deg2rad/2.),2.)  );
      //val = mottxs * (1./eta) * ( (Q2eff/q2_3)*pow(FB_FC_sum,2.) + (pow(mu3H,2.0)*Q2eff/(2*pow(Mt3H,2)*GeV2fm))*(0.5*Q2eff/q2_3 + pow(tan(theta*deg2rad/2),2))*pow(FB_FM_sum,2.) ); //SOG XS formula.
      val = mottxs * (1/eta) * (  pow(FB_FC_sum,2.)/(1+tau) + tau*pow(mu3H,2)*pow(FB_FM_sum,2.)*( 1/(1+tau) + 2*pow(tan(theta*deg2rad/2),2.) )  ); //Beck 1984 XS formula.
    }
  return val;
}
...
void Global_Fit_3H_SOG()
{
  ...
  if(usedifmin == 0)
    {
      ...
      fp = fopen("/w/halla-scifs17exp/triton/skbarcus/Analysis/Global_Fits/Tritium_Data_Amroun.txt","r");
    }
  ...
  if(usedifmin == 1)
    {//File *fp = new File;
      ...
      //fp = fopen("/home/skbarcus/Tritium/Analysis/SOG/Tritium_Data_Amroun.txt","r");
      fp = fopen("/w/halla-scifs17exp/triton/skbarcus/Analysis/Global_Fits/Tritium_Data_Amroun.txt","r");
  ...
  std::ofstream output ("Ri_Chi2_3H.txt", std::ofstream::out);
  ...
  for(Int_t q=0;q<loops;q++)
      ...
      if(userand == 4) //ngaus = 8
	{
	  ...
	  R[4] = TMath::Nint(rand4->GetRandom())/10.+R[3];
	  gRandom->SetSeed(0);                    //Sets new random seed.
	  TF1 *rand5 = new TF1("rand5","x",5.,6.);
	  R[5] = TMath::Nint(rand5->GetRandom())/10.+R[4];
	  gRandom->SetSeed(0);                    //Sets new random seed.
	  TF1 *rand6 = new TF1("rand6","x",5.,6.);
	  R[6] = TMath::Nint(rand6->GetRandom())/10.+R[5];
	  gRandom->SetSeed(0);                    //Sets new random seed.
	  TF1 *rand7 = new TF1("rand7","x",5.,6.);
	  R[7] = TMath::Nint(rand7->GetRandom())/10.+R[6];
      ...
      if(showplots == 1)
	{
	  ...
	  for(Int_t h=0;h<245;h++)
	    {
	      //cout<<"Chi2["<<h<<"] = "<<Chi2[h]<<endl;
	    }
	  cout<<"Amroun_pts = "<<Amroun_pts<<endl;
	  //Plot 150 Amroun data points. 
	  for (Int_t i=0;i<Amroun_pts;i++) 
	    {
	      m1 = new TMarker(Q2[i], Chi2[i], 20);
	      m1->SetMarkerColor(2);
	      m1->SetMarkerSize(1);
	      m1->Draw();
	    }
	  ...
	  for (Int_t i=0;i<Amroun_pts;i++) 
	    {
	      m1 = new TMarker(theta[i], sigexp[i]/xsfit[i], 20);
	      m1->SetMarkerColor(2);
	      m1->SetMarkerSize(1);
	      m1->Draw();
	    }
	  ...
	  for (Int_t i=Amroun_pts;i<(Amroun_pts+Collard_pts);i++) 
	    {
	      m2 = new TMarker(theta[i], sigexp[i]/xsfit[i], 20);
	      m2->SetMarkerColor(4);
	      m2->SetMarkerSize(1);
	      m2->Draw();
	    }
	  ...
	  for (Int_t i=(Amroun_pts+Collard_pts);i<(Amroun_pts+Collard_pts+Beck_1984_pts);i++) 
	    {
	      m3 = new TMarker(theta[i], sigexp[i]/xsfit[i], 20);
	      m3->SetMarkerColor(3);
	      m3->SetMarkerSize(1);
	      m3->Draw();
	    }
	  ...
	  for (Int_t i=(Amroun_pts+Collard_pts+Beck_1984_pts);i<(Amroun_pts+Collard_pts+Beck_1984_pts+Beck_1982_pts);i++) 
	    {
	      m4 = new TMarker(theta[i], sigexp[i]/xsfit[i], 20);
	      m4->SetMarkerColor(6);
	      m4->SetMarkerSize(1);
	      m4->Draw();
	    }
	  ...
	  for (Int_t i=0;i<Amroun_pts;i++) 
	    {
	      m1 = new TMarker(pow(Q2[i],0.5), sigexp[i]/xsfit[i], 20);
	      m1->SetMarkerColor(2);
	      m1->SetMarkerSize(1);
	      m1->Draw();
	    }
	  ...
	  for (Int_t i=Amroun_pts;i<(Amroun_pts+Collard_pts);i++) 
	    {
	      m2 = new TMarker(pow(Q2[i],0.5), sigexp[i]/xsfit[i], 20);
	      m2->SetMarkerColor(4);
	      m2->SetMarkerSize(1);
	      m2->Draw();
	    }
	  ...
	  for (Int_t i=(Amroun_pts+Collard_pts);i<(Amroun_pts+Collard_pts+Beck_1984_pts);i++) 
	    {
	      m3 = new TMarker(pow(Q2[i],0.5), sigexp[i]/xsfit[i], 20);
	      m3->SetMarkerColor(3);
	      m3->SetMarkerSize(1);
	      m3->Draw();
	    }
	  ...
	  for (Int_t i=(Amroun_pts+Collard_pts+Beck_1984_pts);i<(Amroun_pts+Collard_pts+Beck_1984_pts+Beck_1982_pts);i++) 
	    {
	      m4 = new TMarker(pow(Q2[i],0.5), sigexp[i]/xsfit[i], 20);
	      m4->SetMarkerColor(6);
	      m4->SetMarkerSize(1);
	      m4->Draw();
	    }
  ...
  if(useFB == 1)
	...
	{ 
	  ...
	  for (Int_t i=0;i<Amroun_pts;i++) 
	    {
	      m1 = new TMarker(pow(Q2[i],0.5), residual_FB[i], 20);
	      m1->SetMarkerColor(2);
	      m1->SetMarkerSize(1);
	      m1->Draw();
	    }
	  ...
	  for (Int_t i=Amroun_pts;i<(Amroun_pts+Collard_pts);i++) 
	    {
	      m2 = new TMarker(pow(Q2[i],0.5), residual_FB[i], 20);
	      m2->SetMarkerColor(4);
	      m2->SetMarkerSize(1);
	      m2->Draw();
	    }
	  ...
	  for (Int_t i=(Amroun_pts+Collard_pts);i<(Amroun_pts+Collard_pts+Beck_1984_pts);i++) 
	    {
	      m3 = new TMarker(pow(Q2[i],0.5), residual_FB[i], 20);
	      m3->SetMarkerColor(3);
	      m3->SetMarkerSize(1);
	      m3->Draw();
	    }
	  ...
	  for (Int_t i=(Amroun_pts+Collard_pts+Beck_1984_pts);i<(Amroun_pts+Collard_pts+Beck_1984_pts+Beck_1982_pts);i++) 
	    {
	      m4 = new TMarker(pow(Q2[i],0.5), residual_FB[i], 20);
	      m4->SetMarkerColor(6);
	      m4->SetMarkerSize(1);
	      m4->Draw();
	    }
