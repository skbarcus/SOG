...
void C12_STANF_374_SOG() 
{
 ...
 //Make a new TGraph to plot on the canvas. Parameters are (number of points, x values array, y values array).
  ...
  Double_t fitch(Double_t *Q2, Double_t *par)
  {
	...
	sumH3chtemp = (par[i]/(1.0+2.0*pow(par[ngaus+i],2.0)/pow(gamma,2.0))) * ( cos(pow(Q2[0],0.5)*par[ngaus+i]) + (2.0*pow(par[ngaus+i],2.0)/pow(gamma,2.0)) * (sin(pow(Q2[0],0.5)*par[ngaus+i])/(pow(Q2[0],0.5)*par[ngaus+i])) );
...
*/
  ...
  /*
Double_t mottxs(Double_t *Q2, Double_t *par)
  {
    Double_t val = 0; 
    Double_t angle = 0.;
    val = (  (pow(1,2)*(Ef/E0)) * (pow(alpha,2.0)/(4.0*pow(E0,2.0)*pow(sin(angle/2.0),4.0)))*pow(cos(angle/2.0),2.0)  ) * 1.0/25.7;    //Convert GeV^-2 to fm^2 by multiplying by 1/25.7.
