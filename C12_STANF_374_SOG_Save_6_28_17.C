...
Double_t gamma = 0.8/pow(3./2.,0.5);//1.2*pow(2.0/3.0,0.5);   //Gaussian width [fm] from Amroun gamma*sqrt(3/2) = 0.8 fm.
...
*/
 ...
 //c2->SetLogy(); 
 graph->Draw();
 TF1 *fxs = new TF1("fxs",xs, ymin, ymax,ngaus*2);//ngaus*2);
...
*/
 ...
 for(Int_t i=0;i<ngaus*2;i++)
   {
   }

 Double_t ChFF(Double_t *angle, Double_t *par)
 {
