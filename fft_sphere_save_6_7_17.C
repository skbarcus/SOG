...
//********* Data array - same transform ********//
  ...
  */
  ...
  for (Int_t i=0; i<=nback*1.; i++)
    {
      x = ymin + (Double_t(i)/(nback*1.))*(fitrange+999.);//range;  //Only fill bins up to max of fitted range.
      //cout<<x<<endl;
      if(x==0.)
	{
  ...
  */
  ...
  TVirtualFFT *iFFT = TVirtualFFT::FFT(1, &ndim, "C2CBACKWARD M K");   //C2R and C2CBACKWARD both seem to give the same results.
