...
//********* Histograms ********//
   ...
   TF1 *fsin = new TF1("fsin", "cos(2*pi*(3*(x)))*exp(-pi*pow((x),2.0))", ymin, ymax);

   //TF1 *fsin = new TF1("fsin", "cos(2*pi*(5*(x)))*exp(-pi*pow((x),2.0))", ymin, ymax);
   //TF1 *fsin = new TF1("fsin", "exp(-0.5*pow((x),2.0))", ymin, ymax);
   //TF1 *fsin = new TF1("fsin", "1./pow((1.+x),2.)", ymin, ymax);
...
//********* Data array - same transform ********//
  ...
  TH1 *FFT = new TH1D("FFT", "FFT", n+1, -(n+1)/(range*2.), (n+1)/(range*2.));
  ...
  for(Int_t i=0;i<n+2;i++)
    {
      if(i>inflection+1)
	{
	  FFT->SetBinContent(i-1-inflection,hm->GetBinContent(i)/(1./(range/(n+1.))));//*(1./pow(n,0.5)));
	}
    }
  ...
  for(Int_t i=0;i<n;i++)
   {
     if(i<=inflection)
       {
	 FFT->SetBinContent(inflection+i+1,hm->GetBinContent(i+1)/(1./(range/(n+1.))));//*(1./pow(n,0.5)));
       }
   }
